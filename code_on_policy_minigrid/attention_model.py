...
def dot_scaled_attention(
	...
	seq_len, _, d_k = query_shape

	QK_t_scaled = torch.bmm(key.permute(1, 0, 2), query.permute(1, 2, 0)) / math.sqrt(d_k)  # (batch_size, sequence_length, sequence_length)
	# QK_t_scaled = torch.einsum("abc,acd->abd", (key.permute(1,0,2), query.permute(1,2,0))) / math.sqrt(d_k)
	
	# Calculate softmax along the axis of same query (not key(dim 2) ).
	distribution = nn.functional.softmax(QK_t_scaled, dim=1)  # (batch_size, sequence_length, sequence_length)
	
	attention = torch.bmm(value.permute(1, 2, 0), distribution).permute(2, 0, 1)
	# attention: torch.Tensor = torch.einsum("abc,acd->abd", (value.permute(1,2,0), distribution)).permute(2,0,1)
...
class MultiHeadAttention(nn.Module):
	...
	def forward(self,
		...
		attention_stacked, distribution = dot_scaled_attention(query=Q_embed_concat, key=K_embed_concat, value=V_embed_concat)
		
		attention = self.O_linear(torch.cat(attention_stacked.permute(1, 0, 2).split(batch_size, dim=0), 2)).permute(1,0,2)
